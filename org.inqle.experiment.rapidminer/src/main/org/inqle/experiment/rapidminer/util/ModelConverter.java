...
import org.inqle.data.rdf.jena.util.ArcTable;
import org.inqle.data.rdf.jena.util.ArcTableFactory;
...
//					log.trace("Get mapping for non-numeric attribute: " + attribute + "; ALL VALUES=" + arcTable.getColumnValues(columnIndex));
	...
	private List<Attribute> getAttributes(ArcTable arcTable) {
		...
		for (int i=0; i<columnCount; i++) {
			...
			if (columnType==ArcTable.COLUMN_TYPE_NO_VALUES || dataType==ArcTable.DATA_TYPE_NO_VALUES) {
				rapidMinerDataType = Ontology.SINGLE_VALUE;
			} else if (columnType==ArcTable.COLUMN_TYPE_ONE_VALUE) {
				rapidMinerDataType = Ontology.SINGLE_VALUE;
			} else if (columnType==ArcTable.COLUMN_TYPE_ALL_UNIQUE && dataType==ArcTable.DATA_TYPE_STRING) {
				rapidMinerDataType = Ontology.STRING;
			} else if (dataType==ArcTable.DATA_TYPE_DATE) {
				rapidMinerDataType = Ontology.DATE_TIME;
			} else if(dataType==ArcTable.DATA_TYPE_NUMERIC) {
